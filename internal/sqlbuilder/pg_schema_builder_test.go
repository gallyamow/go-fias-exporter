...
func TestPostgreSQLSchemaBuilder_Build(t *testing.T) {
...
`
		...
		want := `CREATE TABLE addressobjects (
...
COMMENT ON TABLE addressobjects IS 'Состав и структура файла со сведениями классификатора адресообразующих элементов БД ФИАС';
COMMENT ON COLUMN addressobjects.id IS 'Уникальный идентификатор записи. Ключевое поле';
COMMENT ON COLUMN addressobjects.objectid IS 'Глобальный уникальный идентификатор адресного объекта типа INTEGER';
COMMENT ON COLUMN addressobjects.objectguid IS 'Глобальный уникальный идентификатор адресного объекта типа UUID';
COMMENT ON COLUMN addressobjects.changeid IS 'ID изменившей транзакции';
COMMENT ON COLUMN addressobjects.name IS 'Наименование';
COMMENT ON COLUMN addressobjects.typename IS 'Краткое наименование типа объекта';
COMMENT ON COLUMN addressobjects.level IS 'Уровень адресного объекта';
COMMENT ON COLUMN addressobjects.opertypeid IS 'Статус действия над записью – причина появления записи';
COMMENT ON COLUMN addressobjects.previd IS 'Идентификатор записи связывания с предыдущей исторической записью';
COMMENT ON COLUMN addressobjects.nextid IS 'Идентификатор записи связывания с последующей исторической записью';
COMMENT ON COLUMN addressobjects.updatedate IS 'Дата внесения (обновления) записи';
COMMENT ON COLUMN addressobjects.startdate IS 'Начало действия записи';
COMMENT ON COLUMN addressobjects.enddate IS 'Окончание действия записи';
COMMENT ON COLUMN addressobjects.isactual IS 'Статус актуальности адресного объекта ФИАС';
COMMENT ON COLUMN addressobjects.isactive IS 'Признак действующего адресного объекта';`
...
`
		...
		builder := NewPostgreSQLSchemaBuilder("tmp", "normative_docs_kinds", false, false)
...
`
		...
		builder := NewPostgreSQLSchemaBuilder("tmp", "normative_docs_types", false, false)
...
COMMENT ON COLUMN tmp.normative_docs_types.enddate IS 'Дата окончания действия записи';`
	...
	t.Run("normative_docs", func(t *testing.T) {
		xmlData := `<?xml version="1.0" encoding="utf-8"?>
<!-- edited with XMLSpy v2011 rel. 2 (http://www.altova.com) by TeaM DJiNN (TeaM DJiNN) -->
...
`
		...
		builder := NewPostgreSQLSchemaBuilder("tmp", "normative_docs", false, false)
...
`
		...
		builder := NewPostgreSQLSchemaBuilder("tmp", "normative_docs", true, false)
...
`
		...
		builder := NewPostgreSQLSchemaBuilder("tmp", "normative_docs", true, true)
