...
func TestUpsertBuilder_Build(t *testing.T) {
	...
	t.Run("no primary key", func(t *testing.T) {
		...
		want := `INSERT INTO test_table (field,name) VALUES ('1','Alice'),('2','Bob')`
		if got != want {
			t.Fatalf("got %s, want %s", got, want)
		}
	})
	...
	t.Run("escape values", func(t *testing.T) {
		...
		want := `INSERT INTO test_table (id,name) VALUES ('1','Alice''s brother'),('2','Bob''s car') ON CONFLICT (id) DO UPDATE SET name=EXCLUDED.name`
		if got != want {
			t.Fatalf("got %s, want %s", got, want)
		}
	})
