...
func TestSchemaBuilderPSQL_Build(t *testing.T) {
...
`
		...
		builder := NewSchemaBuilder("tmp", "addr_obj", false, &PSQLDriver{})
...
`
		...
		builder := NewSchemaBuilder("tmp", "normative_docs_kinds", false, &PSQLDriver{})
...
`
		...
		builder := NewSchemaBuilder("tmp", "normative_docs_types", false, &PSQLDriver{})
...
`
		...
		builder := NewSchemaBuilder("tmp", "normative_docs", false, &PSQLDriver{})
...
`
		...
		builder := NewSchemaBuilder("tmp", "normative_docs", true, &PSQLDriver{})
...
func TestSchemaBuilderMySQL_Build(t *testing.T) {
	t.Run("basic", func(t *testing.T) {
		xmlData := `<?xml version="1.0" encoding="utf-8"?>
<!-- edited with XMLSpy v2011 rel. 2 (http://www.altova.com) by TeaM DJiNN (TeaM DJiNN) -->
<xs:dbSchema xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:sch="http://purl.oclc.org/dsdl/schematron" xmlns:usch="http://www.unisoftware.ru/schematron-extensions" xmlns:sql="urn:schemas-microsoft-com:mapping-dbSchema" elementFormDefault="qualified" attributeFormDefault="unqualified">
...
`
		...
		builder := NewSchemaBuilder("", "addr_obj", false, &MySQLDriver{})
		...
		want := `CREATE TABLE addr_obj (
	id VARCHAR(255) NOT NULL PRIMARY KEY COMMENT 'Уникальный идентификатор записи. Ключевое поле',
	objectid VARCHAR(255) NOT NULL COMMENT 'Глобальный уникальный идентификатор адресного объекта типа INTEGER',
	objectguid VARCHAR(255) NOT NULL COMMENT 'Глобальный уникальный идентификатор адресного объекта типа UUID',
	changeid VARCHAR(255) NOT NULL COMMENT 'ID изменившей транзакции',
	name VARCHAR(255) NOT NULL COMMENT 'Наименование',
	typename VARCHAR(255) NOT NULL COMMENT 'Краткое наименование типа объекта',
	level VARCHAR(255) NOT NULL COMMENT 'Уровень адресного объекта',
	opertypeid VARCHAR(255) NOT NULL COMMENT 'Статус действия над записью – причина появления записи',
	previd VARCHAR(255) COMMENT 'Идентификатор записи связывания с предыдущей исторической записью',
	nextid VARCHAR(255) COMMENT 'Идентификатор записи связывания с последующей исторической записью',
	...
	isactual VARCHAR(255) NOT NULL COMMENT 'Статус актуальности адресного объекта ФИАС',
	isactive VARCHAR(255) NOT NULL  COMMENT 'Признак действующего адресного объекта'
) COMMENT='Состав и структура файла со сведениями классификатора адресообразующих элементов БД ФИАС';`
